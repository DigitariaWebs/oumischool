...
const mockTutorSessions: TutorSession[] = [
  {
    ...
    status: "completed",
    rating: 5,
    notes: "Excellente séance sur les fractions. Emma a bien progressé!",
  },
  {
    id: "session-2",
    tutorId: "1",
    ...
    status: "completed",
    rating: 5,
    notes: "Très bonne compréhension de la conjugaison.",
  },
  {
    id: "session-3",
    tutorId: "2",
...
export default function ChildDetailsScreen() {
  ...
  const getSessionStatusColor = (status: TutorSession["status"]) => {
    switch (status) {
      case "completed":
  ...
  const getSessionStatusText = (status: TutorSession["status"]) => {
    switch (status) {
      case "completed":
        return "Terminée";
      case "upcoming":
      ...
      >
                ...
                {daySchedule.sessions.length > 0 ? (
                  daySchedule.sessions.map((session, sessionIndex) => (
                    <View
                      key={session.id}
                      style={[
                        styles.scheduleSession,
                        sessionIndex > 0 && { marginTop: 8 },
                      ]}
                    >
                      <View style={styles.scheduleSessionLeft}>
                        <Image
                          source={{ uri: session.tutorAvatar }}
                          style={styles.scheduleTutorAvatar}
                        />
                        <View style={styles.scheduleSessionInfo}>
                          <Text style={styles.scheduleSessionTime}>
                            {session.time}
                          </Text>
                          <Text style={styles.scheduleTutorName}>
                            {session.tutorName}
                          </Text>
                          <View
                            style={[
                              styles.scheduleSubjectBadge,
                              {
                  ...
                  <View style={styles.emptyDaySchedule}>
        ...
        >
          <Text style={styles.sectionTitle}>
            Historique des séances avec tuteurs
          </Text>
          <View style={styles.sessionsContainer}>
            {mockTutorSessions.map((session, index) => (
              <View key={session.id} style={styles.sessionCard}>
                <View style={styles.sessionHeader}>
                  <View style={styles.sessionTutorInfo}>
                    ...
                    <View style={styles.sessionDetails}>
                      <Text style={styles.sessionTutorName}>
                        {session.tutorName}
                      </Text>
                      <View
                        style={[
                  ...
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          getSessionStatusColor(session.status) + "20",
                      },
                    ]}
                  >
                    <Text
                ...
                <View style={styles.sessionInfo}>
                  ...
                  <View style={styles.sessionInfoItem}>
                    <Clock size={14} color={COLORS.secondary[600]} />
                    <Text style={styles.sessionInfoText}>
                  ...
                  <View style={styles.sessionInfoItem}>
                    {session.mode === "online" ? (
                      <Video size={14} color={COLORS.secondary[600]} />
                    ) : (
                ...
                {session.status === "completed" && session.rating && (
                  <View style={styles.sessionRating}>
                    <View style={styles.sessionRatingStars}>
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          size={16}
                          color="#F59E0B"
                          fill={i < session.rating! ? "#F59E0B" : "transparent"}
                        />
                      ))}
                    </View>
                    {session.notes && (
